...
import type { TreeNode } from '$common/models/ipc-events';
...
class WtfService {
	...
	public async getWtfContents(
		...
		try {
			...
			const tree = await fileService.getDirectoryTree(wtfPath, { includeHash });
	...
	public async listAccountPaths(installations: WowInstallation): Promise<string[]> {
		return await fileService.listDirectories(this.getAccountsPath(installations));
	}
	...
	public async getServers(installation: WowInstallation, account: string): Promise<string[]> {
		...
		const entries = await fileService.listDirectories(accountPath);
	...
	public async getCharacters(
		...
		const entries = await fileService.listDirectories(serverPath);
		return entries;
	}
	...
	public async getCharacterVariables(
		...
		try {
			...
			const entries = await fileService.listFiles(characterPath, '*.lua');

			const entryPaths = entries.map((entry) => path.join(characterPath, entry));
			const fileSizes = await fileService.statFiles(entryPaths);
	...
	public async getGlobalVariables(
		...
		const entries = await fileService.listFiles(accountPath, '*.lua');
	...
	public async getBackupList(installation: WowInstallation): Promise<WtfBackup[]> {
		...
		try {
			...
			const fsStats = await fileService.statFiles(backupZipFiles);
			...
			for (let i = 0; i < backupZipFiles.length; i++) {
				...
				try {
					const zipMetaTxt = await fileService.readFileInZip(zipFile, BACKUP_META_FILENAME);
	...
	public async createBackup(
		...
		try {
			...
			await fileService.remove(metadataFilePath);
		}
	}
	...
	public async deleteBackup(fileName: string, installation: WowInstallation): Promise<void> {
		...
		const pathExists = await fileService.pathExists(fullPath);
		if (!pathExists) {
			throw new Error('path not found');
		}

		await fileService.remove(fullPath);
	}
	...
	public async applyBackup(fileName: string, installation: WowInstallation): Promise<void> {
		...
		try {
			...
			const wtfPathExists = await fileService.pathExists(wtfPath);
			const wtfBackupPath = `${wtfPath}-wowup`;
			if (wtfPathExists) {
				console.log(`Rename current wtf folder: ${wtfPath}`);
				await fileService.renameFile(wtfPath, wtfBackupPath);
			}

			console.log(`Unzip backup file: ${wtfPath}`);
			try {
				await fileService.unzipFile(fullPath, wtfPath);
				...
				await fileService.deleteIfExists(wtfPath);
				await fileService.renameFile(wtfBackupPath, wtfPath);
				throw e;
			}

			console.log(`Removing soft backup: ${wtfBackupPath}`);
			await fileService.deleteIfExists(wtfBackupPath);
		} finally {
	...
	private async isBackupApplicationValid(zipFile: string, installation: WowInstallation) {
		const srcMetaTxt = await fileService.readFileInZip(zipFile, BACKUP_META_FILENAME);
	...
	private async createBackupZip(installation: WowInstallation): Promise<void> {
		...
		try {
			...
			await fileService.zipFile(wtfPath, zipPath);
		} finally {
	...
	private async createBackupDirectory(installation: WowInstallation): Promise<void> {
		const backupPath = this.getBackupPath(installation);
		await fileService.createDirectory(backupPath);
	}
	...
	private async deleteBackupMetadataFile(installation: WowInstallation) {
		...
		await fileService.deleteIfExists(metaPath);
	}
	...
	private async writeWtfMetadataFile(
		...
		const metaPath = path.join(wtfPath, BACKUP_META_FILENAME);
		await fileService.writeFile(metaPath, JSON.stringify(backupMetadata, null, 2));
	...
	private async listBackupFiles(installation: WowInstallation) {
		const backupPath = this.getBackupPath(installation);
		const zipFiles = await fileService.listFiles(backupPath, '*.zip');
	...
	public getBackupPath(installation: WowInstallation): string {
		return path.join(wowup.wtfBackupFolder, installation.id);
	}
...
export const wtf = new WtfService();
