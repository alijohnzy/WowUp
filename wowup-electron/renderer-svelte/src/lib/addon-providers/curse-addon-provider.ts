...
import { strictFilter } from '$lib/utils/misc';
import { TocNotFoundError } from '$lib/errors';
...
export class CurseAddonProvider extends AddonProvider {
	...
	public override async getAllBatch(
	...
	public override async getFeaturedAddons(
		installation: WowInstallation
	...
	public override async searchProtocol(
		protocol: string
	): Promise<ProtocolSearchResult | undefined> {
		...
		const addonFileResponse = await this.getAddonFileById(
			protocolData.addonId,
			protocolData.fileId
		);
				...
				if (
					...
					exactMatch = fingerprintData.partialMatches.find((partialMatch) =>
						partialMatch.file?.modules?.some(
							(module) => module.fingerprint === af.cfScanResults?.fingerprintNum
						)
					);
				}
	...
	public override async getChangelog(
		...
		try {
			...
			const response = await caching.transaction(
				cacheKey,
				() => {
					return this._cf2Client.getModFileChangelog(
						parseInt(externalId, 10),
						parseInt(externalReleaseId, 10)
					);
				},
	...
	private getLatestFiles(result: cfv2.CF2Addon, clientType: WowClientType): cfv2.CF2File[] {
		const filtered = result.latestFiles.filter(
			(latestFile) =>
				latestFile.exposeAsAlternative !== true && this.isClientType(latestFile, clientType)
		);
	...
	private async getAddonFileById(
	...
	private async getByIdBase(addonId: string): Promise<cfv2.CF2Addon | undefined> {
		try {
			const response = await this._circuitBreaker.fire(() =>
				this._cf2Client.getMod(parseInt(addonId, 10))
			);
			return response.data?.data;
		} catch (e) {
	...
	private getAuthor(result: cfv2.CF2Addon): string {
		const authorNames = result.authors
			.map((a) => a.name)
			.filter((lf) => !lf.toLowerCase().startsWith('_forgeuser'));
	...
	private getGameVersion(file: cfv2.CF2File): string {
		return file.gameVersions[0] ?? '';
	}

	private createAddonSearchResultDependency = (
		dependency: cfv2.CF2FileDependency
	): AddonSearchResultDependency => {
		return {
	...
	private toAddonDependencyType(
		curseDependencyType: cfv2.CF2FileRelationType
	): AddonDependencyType {
	...
	private getAddonSearchResult(
		result: cfv2.CF2Addon,
		latestFiles: cfv2.CF2File[] = []
	...
	private async getFeaturedAddonList(wowInstallation: WowInstallation): Promise<cfv2.CF2Addon[]> {
		...
		const result = await caching.transaction(
			cacheKey,
		...
		if (body) {
			...
			const uniqueRecent = body.recentlyUpdated.filter(
				(ru) => !body.popular.some((p) => p.id === ru.id)
			);
	...
	private async getCategoryAddons(
		...
		const result = await caching.transaction(cacheKey, () =>
