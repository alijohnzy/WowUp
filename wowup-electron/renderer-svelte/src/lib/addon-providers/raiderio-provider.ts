...
import { AddonChannelType, AddonProvider, getGameVersionList } from 'wowup-lib-core';
import type { AddonFolder } from 'wowup-lib-core';
...
export class RaiderIoAddonProvider extends AddonProvider {
	...
	public scan(
		...
		const dependencies = _.filter(addonFolders, (addonFolder) =>
			this.isRaiderIoDependant(addonFolder)
		);
		...
		for (const rioAddonFolder of rioAddonFolders) {
			const subTargetToc = tocService.getTocForGameType2(
				rioAddonFolder.name,
				rioAddonFolder.tocs,
				installation.clientType
			);
	...
	private isRaiderIo(addonFolder: AddonFolder) {
		return (
			...
			addonFolder.tocs.some(
				(toc) => toc.website === this._scanWebsite && toc.addonProvider === this._scanAddonProvider
			)
		);
	}
	...
	private isRaiderIoDependant(addonFolder: AddonFolder) {
		return addonFolder.tocs.some(
			(toc) =>
				toc.dependencies !== undefined && toc.dependencies.indexOf(this._scanFolderName) !== -1
		);
	}
}
