...
import { on as ipcOn } from '$lib/ipc';
...
import * as caching from '$lib/services/caching';
import * as network from '$lib/services/network';
...
import * as uiMessage from '$lib/services/ui-message';
import { ValueNotifier } from '$lib/utils/misc';
import { AddonChannelType, AddonProvider, WowClientGroup, WowClientType } from 'wowup-lib-core';
import type {
	Addon,
	AddonFolder,
	AddonScanResult,
...
export class WagoAddonProvider extends AddonProvider {
	...
	private _apiTokenSrc = new ValueNotifier<string>('');
	private _wagoSecret = '';

	// This is our internal http queue, prevents duplicated requests for some routes.
	// Only getAddonById uses it, so the value type is concrete — it was `Promise<any>`, which
	// meant the `return this._requestQueue.get(url)` below type-checked against anything.
	private _requestQueue: Map<string, Promise<WagoAddon>> = new Map();
	...
	public constructor(private readonly onAdRequirementChanged: () => void = () => {}) {
		...
		ipcOn('wago-token-received', this.onWagoTokenReceived);

		// Watch for the change of the wago secret in the store
		sensitiveStorage.change((change) => {
			if (change.key !== PREF_WAGO_ACCESS_KEY) return;
		...
		electron.powerMonitor.subscribe((state) => {
			// Clear the token on resume so we wait for a fresh one.
			if (state === IPC_POWER_MONITOR_RESUME) this._apiTokenSrc.set('');
		});
	}
	...
	public override async scan(
		...
		scanResults.forEach((res) => {
			...
			const toc = tocService.getTocForGameType2(
				addonFolder.name,
				addonFolder.tocs,
		...
		for (const scanResult of scanResults) {
			try {
				const fingerprintMatches = matchResult.addons.filter((addon) => {
					...
					const mods = Object.values(addon.modules ?? {}).filter(
						(mod) => typeof mod.hash === 'string'
					);
	...
	private async sendRequest<T>(action: () => Promise<T>): Promise<T> {
		try {
			...
			if (err instanceof HttpError) {
				...
				uiMessage.sendMessage('ad-frame-reload');
			}
			throw err;
		}
	}
	...
	public override async getAll(
		...
		const response = await this.sendRequest(() =>
			caching.transaction(
				`${installation.id}|${url.toString()}`,
				() =>
					this._circuitBreaker.postJson<WagoRecentsResponse>(
						url,
						request,
	...
	private async getAddonById(addonId: string): Promise<WagoAddon> {
		...
		const queued = this._requestQueue.get(url);
		if (queued !== undefined) {
			return await queued;
		}
	...
	private toSearchResultFile(
		...
		return {
			...
			downloadUrl:
				('download_link' in release ? release.download_link : undefined) ??
				('link' in release ? release.link : undefined) ??
				'',
			folders: [],
			gameVersion: '',
	...
	private getSortedReleaseList(wagoScanAddon: WagoScanAddon): WagoScanReleaseSortable[] {
		...
		for (const [key, value] of Object.entries(wagoScanAddon.recent_releases)) {
			releaseList.push({
				...value,
				stability: key,
				addonChannelType: this.getAddonChannelType(key as WagoStability)
			});
		}
	...
	private onWagoTokenReceived = (_evt: unknown, token: string) => {
	...
	//
	private async ensureToken(timeoutMs = 10000): Promise<string> {
		...
		if (this.isValidToken(this._wagoSecret)) {
			...
			return this._wagoSecret;
		}

		// Otherwise wait for a public token pushed in from the ad frame.
		const token = await this._apiTokenSrc.waitFor((t) => this.isValidToken(t), timeoutMs, '');
		if (token === '') {
			console.error('[wago] no token received after timeout');
		} else {
			console.log(`[wago] token ready`, token.length);
		}
		return token;
	}
}
