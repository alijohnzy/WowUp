...
import { mapWithConcurrency, strictFilterBy } from '$lib/utils/misc';
import { sensitiveStorage } from '$lib/services/storage';
import { AddonChannelType, AddonProvider, WowClientType } from 'wowup-lib-core';
import type {
	AddonSearchResult,
...
import type { GitHubAsset, GitHubRelease, GitHubRepository, WowInstallation } from 'wowup-lib-core';
...
export class GitHubAddonProvider extends AddonProvider {
	...
	public async getAll(installation: WowInstallation, addonIds: string[]): Promise<GetAllResult> {
		// Was rxjs mergeMap with a concurrency of 3.
		const taskResults = await mapWithConcurrency(
			addonIds,
			(addonId) => this.handleGetAllItem(addonId, installation),
			3
		);

		const result: GetAllResult = {
			errors: _.concat(
				taskResults.map((tr) => tr.error).filter((e): e is Error => e !== undefined)
			),
			searchResults: _.concat(
				taskResults
					.map((tr) => tr.searchResult)
	...
	public async searchByUrl(
		addonUri: URL,
		installation: WowInstallation
	): Promise<SearchByUrlResult> {
		...
		try {
			...
			const potentialAddon: AddonSearchResult = {
				...
				files: [
					{
						channelType: result.release?.prerelease
	...
	private async getLatestValidAsset(
		...
		let latestRelease: GitHubRelease | undefined = sortedReleases[0];
	...
	private getLatestRelease(releases: GitHubRelease[]): GitHubRelease {
		...
		const firstItem = sortedReleases[0];
	...
	private getValidAssetFromMetadata(
		...
		try {
			...
			const targetMetaRelease = releaseMeta.releases.find(
				(release) =>
					release.nolib === false &&
					release.metadata.findIndex((m) => m.flavor === targetFlavor) !== -1
			);
	...
	private getValidAssetForAny(release: GitHubRelease | undefined): GitHubAsset | undefined {
		...
		const sortedAssets = _.filter(
			release.assets,
			(asset) => this.isNotNoLib(asset) && this.isValidContentType(asset)
		);
		return sortedAssets[0];
	}
	...
	private isValidContentType(asset: GitHubAsset): boolean {
		...
		if (
			RELEASE_CONTENT_TYPES.OCTET_STREAM === asset.content_type &&
	...
	private handleRateLimitError(response: HttpError) {
	...
	private handleNotFoundError(response: HttpError) {
		if (response.status === 404) {
			throw new SourceRemovedAddonError('', response);
		}
	}

	// Was @angular/common/http HttpHeaders; the fetch Headers API has the same get().
	private getIntHeader(headers: Headers, key: string) {
		return parseInt(headers.get(key) ?? '', 10);
	}

	private async hasPersonalAccessKey(): Promise<boolean> {
		const pat = await sensitiveStorage.getAsync(PREF_GITHUB_PERSONAL_ACCESS_TOKEN);
		return typeof pat === 'string' && pat.length > 0;
	}

	private async getAuthorizationHeader(): Promise<{ [param: string]: string }> {
		const personalAccessToken = await sensitiveStorage.getAsync(PREF_GITHUB_PERSONAL_ACCESS_TOKEN);
